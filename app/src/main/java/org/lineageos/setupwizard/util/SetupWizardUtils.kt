...
object SetupWizardUtils {
    ...
    fun hasRecoveryUpdater(context: Context): Boolean {
        ...
        try {
            ...
            val res =
                updaterResources.getIdentifier(CONFIG_HIDE_RECOVERY_UPDATE, "bool", UPDATER_PACKAGE)
            featureHidden = updaterResources.getBoolean(res)
        } catch (ignored: Exception) {}
        return !featureHidden
    }

    @JvmStatic fun isOwner(): Boolean = UserHandle.myUserId() == 0
    ...
    @JvmStatic fun getBuildDateTimestamp(): Long = SystemProperties.getLong(PROP_BUILD_DATE, 0)
