...
class WizardScript(actions: Map<String, WizardAction>, val firstActionId: String) : Parcelable {
    ...
    fun getNextActionId(currentActionId: String, resultCode: Int): String? {
        ...
        if (resultCode != Activity.RESULT_CANCELED) {
            ...
            Log.v(
                TAG,
                "getNextActionId($currentActionId,$resultCode) " +
        ...
        fun loadFromUri(context: Context, uriString: String): WizardScript? {
            return runCatching {
                    ...
                    val parser: XmlPullParser =
                        if (res.r.getResourceTypeName(res.id) == "xml") {
                            res.r.getXml(res.id)
                        } else {
                            val inputStream: InputStream = res.r.openRawResource(res.id)
                ...
                .getOrElse { e ->
                    val message =
                        when (e) {
                            is XmlPullParserException -> "Ill-formatted wizard_script: $uriString"
                            is FileNotFoundException -> "Cannot find file: $uriString"
                            is IOException -> "Unable to read wizard_script: $uriString"
                            else -> throw e
                        }
                    Log.e(TAG, message)
                    Log.e(TAG, e.message ?: "")
