...
import org.lineageos.setupwizard.ACTION_LOAD
import org.lineageos.setupwizard.EXTRA_ACTION_ID
import org.lineageos.setupwizard.EXTRA_RESULT_CODE
...
class WizardManager : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (LOGV) {
            Log.v(
                TAG,
                "  action=$action resultCode=$resultCode scriptUri=$scriptUri " +
                    "actionId=$actionId extras=${intent.extras}",
            )
        }
    ...
    private fun doAction(scriptUri: String, action: WizardAction, extras: Intent?) {
        ...
        if (LOGV) {
            Log.v(
                TAG,
                "doAction scriptUri=$scriptUri extras=$extras intent=$intent " +
                    "extras2=${intent.extras} action=$action",
            )
        }
    ...
    private fun load(scriptUri: String, extras: Intent?) {
        ...
        if (wizardAction != null) {
            ...
            Log.e(
                TAG,
                "load could not resolve first action scriptUri=$scriptUri " +
                    "actionId=${wizardScript.getFirstActionId()}",
            )
            exit(scriptUri)
        }
    }
    ...
    companion object {
        ...
        private fun checkNextAction(
            ...
            if (LOGV) {
                Log.v(
                    TAG,
                    "checkNextAction scriptUri=$scriptUri actionId=$actionId " +
