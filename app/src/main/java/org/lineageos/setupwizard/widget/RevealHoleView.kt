...
class RevealHoleView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private companion object {
        fun resolveBackgroundColor(context: Context) =
            context.obtainStyledAttributes(intArrayOf(android.R.attr.colorBackground)).use {
                it.getColor(0, Color.BLACK)
            }
    }

    private val centerX
        get() = if (holeCenterX.isNaN()) width / 2f else holeCenterX

    private val centerY
        get() = if (holeCenterY.isNaN()) height / 2f else holeCenterY
