...
class LineageSettingsActivity : BaseSetupWizardActivity() {
    ...
    private val mMetricsClickListener = View.OnClickListener {
    ...
    private val mNavKeysClickListener = View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val servicesFullDescription = getString(
            R.string.services_full_description, privacyPolicy, policySummary
        )
        ...
        val metricsSummary = getString(
            R.string.services_metrics_label, metricsHelpImproveLineage, osName, osName
        )
        val metricsSpan = SpannableStringBuilder(metricsSummary)
        metricsSpan.setSpan(
            StyleSpan(Typeface.BOLD),
            0, metricsHelpImproveLineage.length,
        ...
        if (mSupportsKeyDisabler) {
            mNavKeys.isChecked = LineageSettings.System.getIntForUser(
                contentResolver,
    ...
    private fun updateMetricsOption() {
        ...
        val metricsChecked = !myPageBundle.containsKey(KEY_SEND_METRICS) ||
    ...
    private fun updateDisableNavkeysOption() {
        if (mSupportsKeyDisabler) {
            val myPageBundle = SetupWizardApp.getSettingsBundle()
            val enabled = LineageSettings.System.getIntForUser(
                contentResolver,
                LineageSettings.System.FORCE_SHOW_NAVBAR,
                0,
                UserHandle.USER_CURRENT
            ) != 0
            val checked = if (myPageBundle.containsKey(DISABLE_NAV_KEYS)) {
