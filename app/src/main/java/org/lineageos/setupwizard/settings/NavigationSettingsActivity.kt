...
class NavigationSettingsActivity : BaseSetupWizardActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val deviceKeys = resources.getInteger(
        ...
        if (!navBarEnabled && hasHomeKey || available <= 1) {
            SetupWizardApp.getSettingsBundle().putString(
                NAVIGATION_OPTION_KEY, NAV_BAR_MODE_3BUTTON_OVERLAY
            )
            finishAction(RESULT_OK)
        }

        val navigationIllustration =
            findViewById<LottieAnimationView>(R.id.navigation_illustration)
