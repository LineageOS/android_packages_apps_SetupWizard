...
class LocaleActivity : BaseSetupWizardActivity() {
    ...
    private lateinit var mLocaleAdapter: ArrayAdapter<com.android.internal.app.LocalePicker.LocaleInfo>
    ...
    private val mSimChangedReceiver = object : BroadcastReceiver() {
    ...
    private fun loadLanguages() {
        mLocaleAdapter = com.android.internal.app.LocalePicker.constructAdapter(
            this, R.layout.locale_picker_item, R.id.locale
        )
            ...
            if (!isFinishing || !isDestroyed) {
                ...
                mHandler.post {
                    if (finalLocale != null && finalLocale != mCurrentLocale) {
                        if (!mSetupWizardApp.ignoreSimLocale() && !isDestroyed) {
                            val label = getString(
                                R.string.sim_locale_changed, finalLocale.displayName
                            )
                            Toast.makeText(
                                this@LocaleActivity, label, Toast.LENGTH_SHORT
                            ).show()
