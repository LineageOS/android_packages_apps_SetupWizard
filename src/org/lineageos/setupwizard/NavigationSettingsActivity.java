...
public class NavigationSettingsActivity extends BaseSetupWizardActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!navBarEnabled && hasHomeKey) {
            mSetupWizardApp.getSettingsBundle().putInt(NAVIGATION_OPTION_KEY, HW_NAVIGATION);
    ...
    protected void onNextPressed() {
        int selection = GESTURE_NAVIGATION;
        switch (((RadioGroup) findViewById(R.id.navigation_radio_group))
                .getCheckedRadioButtonId()) {
            case R.id.radio_gesture:
                selection = GESTURE_NAVIGATION;
                break;
            case R.id.radio_sw_keys:
                selection = NAVBAR_NAVIGATION;
                break;
        }

        mSetupWizardApp.getSettingsBundle().putInt(NAVIGATION_OPTION_KEY, selection);
