...
public class CaptivePortalSetupActivity extends WrapperSubBaseActivity {
    ...
    private static class CheckForCaptivePortalTask extends AsyncTask<Void, Void, Boolean> {
        ...
        public static void checkForCaptivePortal(ConnectivityManager connectivity,
                URL captivePortalUrl, CaptivePortalSetupActivity captivePortalSetupActivity,
                boolean cancelAndRecreateIfRunning) {
            if (sTask == null || sTask.getStatus() == Status.FINISHED) {
                sTask = new CheckForCaptivePortalTask(connectivity, captivePortalUrl,
                        captivePortalSetupActivity);
                sTask.execute();
